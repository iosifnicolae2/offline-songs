...
$('nav.menu-links a').click(function(event) {
  event.preventDefault();
  var album_link = $(this).prop('href');
  window.history.pushState('Album' + $(this).text(), $(this).text(), album_link);
  $(document.body).toggleClass('menu-open');
});

function download_all_songs() {
  console.info('download_all_songs');
  var chars = {'î':'i','ţ':'t','â':'a', 'ă':'a'};
  $.ajax({
      'async': true,
      'global': false,
      'url': '/api/view/all',
      ...
      'success': function (data) {
          ...
          songs_json_array.map((song) => {
            song.full_text = JSON.stringify(song).toLowerCase().replace(/[îţâă]/g, m => chars[m]).replace(/[^\w\s]/gi, '');
            return song;
          });
...
function update_songs_list(search_text) {
  search_text = search_text.replace(/[îţâă]/g, m => chars[m]).replace(/[^\w\s]/gi, '');
  ...
  search_text = search_text.toLowerCase();
    ...
    update_songs_list_view(selected_songs.slice(0,50));
}

function update_songs_list_with_album(album_name) {
  console.log('Album', album_name)
  if(all_songs_have_been_downloaded == false) {
    console.info('songs_loaded_queue')
    songs_loaded_queue.push({'function': update_songs_list_with_album,'param': album_name});
    console.info(songs_loaded_queue)
    download_all_songs();
    return;
  }
    var selected_songs = songs_json_array.filter(function (song) {
        return song.category == album_name;
    });
    console.log('Selected songs for album', album_name, selected_songs)
    // console.info(selected_songs)
...
;(function($) {
  var app = $.sammy(function() {
          ...
          this.get('/view_album', function() {
            update_songs_list_with_album(this.params['id']);
          });
  });
